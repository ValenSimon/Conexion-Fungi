...
function GraciasContent() {
  ...
  useEffect(() => {
    // Validamos contra la sesión local del navegador
    const orderIdSesion = sessionStorage.getItem('ultimo_pedido_fungi');

    if (!orderIdUrl || orderIdUrl !== orderIdSesion) {
      // Si no hay coincidencia, lo mandamos al catálogo tras un breve delay
  ...
  // Pantalla de "Acceso Denegado" o Redirección
    ...
    <div className="min-h-[80vh] bg-[#F8F9FA] flex items-center justify-center px-4 py-12">
      <div className="max-w-md w-full bg-white rounded-[2.5rem] shadow-xl shadow-orange-900/5 p-10 text-center border border-gray-100">
        ...
        <div className="bg-orange-50 rounded-3xl p-6 mb-10 border border-orange-100 relative overflow-hidden">
          ...
          <p className="text-xs text-[#EF8851] uppercase tracking-[0.2em] font-bold mb-1">
        ...
        <div className="flex items-start gap-3 text-left bg-[#EF8851]/5 p-4 rounded-2xl mb-8 border border-[#EF8851]/10">
          ...
          <p className="text-sm  text-gray-900">
            Recuerda que para procesar tu envío debes enviar el <strong>comprobante de transferencia</strong> por WhatsApp. <br /> <strong> Numero : +54(911)60012600 </strong>
          </p>
        ...
        <div className="grid gap-3">
          <Link 
            href="/catalogo" 
            className="flex items-center justify-center gap-2 w-full py-4 px-6 bg-[#EF8851] text-white rounded-2xl font-bold hover:bg-[#d97642] transition-all shadow-lg shadow-orange-500/20 active:scale-[0.98]"
          >
            Seguir explorando
          </Link>
          ...
          <Link 
            href="/" 
            className="flex items-center justify-center gap-2 w-full py-4 px-6 bg-white text-gray-600 border border-gray-200 rounded-2xl font-bold hover:bg-gray-50 transition-all"
          >
        ...
        <p className="mt-10 text-xs text-gray-400">
          ¿Dudas con tu pago? <a href="/contacto" className="text-[#EF8851] font-semibold underline">Escríbenos aquí</a>
        </p>
      </div>
    </div>
  );
...
// Exportación principal con Suspense para evitar errores de Next.js
